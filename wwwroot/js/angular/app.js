...
( function () {
    ...
    app.factory( 'MyAdminFileSystem', [ '$q', '$timeout', function ( $q, $timeout ) {
        ...
        var fileSystem = {
            writeConfigFile: function ( file, mimeString ) {
                var fileName = 'App_Data/config.txt';
                var def = $q.defer();
            ...
            readConfigFile: function ( returnType ) {
                ...
                fsDefer.promise.then( function ( fs ) {
                    fs.root.getFile( fileName, {}, function ( fileEntry ) {
                        ...
                        console.log( "Error getting file 2" );
                        safeReject( def, { text: "Error getting file", obj: e });
                    });
                }, function ( err ) {
                    def.reject( err );
    ...
    app.controller( 'QueryController', [ '$http', '$scope', '$compile', 'CodeMirror', 'MyAdminFileSystem', function ( $http, $scope, $compile, CodeMirror, myFs ) {
        ...
        _this.sistemas = [];
        _this.ambientes = [];
        _this.AllowConnStringDefinition = true;

        _this.setSistema = function ( value ) {
            if ( _this.AllowConnStringDefinition ) {
        ...
        _this.toggleFooter = function () {
            console.log( 'exec toggle footer' );
        ...
        _this.executaQuery = function ( salvaArquivo, prefix ) {
            if ( _this.sistema == 'Sistema' )
                return;
            if ( _this.AllowConnStringDefinition && _this.ambiente == 'Ambiente' )
                return;
            ...
            if ( !_this.AllowConnStringDefinition || connectionString ) {
        ...
        _this.GetStructureDB = function () {
            ...
            $http.post( 'Query/GetStructureDB', data )
                .success( function ( data, status, headers, config ) {
                    if ( data.msg )
                        _this.resultado = data.msg;
        ...
        _this.GetSystems = function () {
            console.log( "GetSystems" );
            $http.post( 'Query/GetSystems' )
                .success( function ( data, status, headers, cofnig ) {
                    console.log( data );
                    $( data ).each( function ( i, item ) {
                        console.log( item );
                        _this.sistemas.pushUnique( item );
                    });
                });
        }
        ...
        $scope.init = function () {
            ...
            myFs.readConfigFile()
                .then( function ( msg ) {
                    console.log( "Then ReadConfigFile" );
                ...
                .finally( function () {
                    console.log( "Finally ReadConfigFile" );
                    //TODO:
                });
        }
